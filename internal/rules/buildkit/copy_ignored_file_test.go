...
func TestIsURLCopyIgnored(t *testing.T) {
	...
	for _, tc := range tests {
		got := isURLCopyIgnored(tc.path)
		if got != tc.want {
			t.Errorf("isURLCopyIgnored(%q) = %v, want %v", tc.path, got, tc.want)
		}
	}
}
...
func TestNormalizePathCopyIgnored(t *testing.T) {
	...
	for _, tc := range tests {
		got := normalizePathCopyIgnored(tc.path)
		if got != tc.want {
			t.Errorf("normalizePathCopyIgnored(%q) = %q, want %q", tc.path, got, tc.want)
		}
	}
}
