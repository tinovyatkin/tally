...
func TestIsForbiddenOnbuildTrigger(t *testing.T) {
	tests := []struct {
		instruction string
		want        bool
	}{
	...
	for _, tt := range tests {
		t.Run(tt.instruction, func(t *testing.T) {
			got := IsForbiddenOnbuildTrigger(tt.instruction)
			if got != tt.want {
				t.Errorf("IsForbiddenOnbuildTrigger(%q) = %v, want %v", tt.instruction, got, tt.want)
			}
		})
	}
}

func TestDL3043_Constants(t *testing.T) {
	if DL3043Code != "hadolint/DL3043" {
		t.Errorf("DL3043Code = %q, want %q", DL3043Code, "hadolint/DL3043")
	}
	if DL3043Message == "" {
		t.Error("DL3043Message should not be empty")
	}
	if DL3043DocURL != "https://github.com/hadolint/hadolint/wiki/DL3043" {
		t.Errorf("DL3043DocURL = %q, want wiki URL", DL3043DocURL)
	}
}
