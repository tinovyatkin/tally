...
func TestIsValidFirstInstruction(t *testing.T) {
	tests := []struct {
		instruction string
		want        bool
	}{
	...
	for _, tt := range tests {
		t.Run(tt.instruction, func(t *testing.T) {
			got := IsValidFirstInstruction(tt.instruction)
			if got != tt.want {
				t.Errorf("IsValidFirstInstruction(%q) = %v, want %v", tt.instruction, got, tt.want)
			}
		})
	}
}

func TestDL3061_Constants(t *testing.T) {
	if DL3061Code != "hadolint/DL3061" {
		t.Errorf("DL3061Code = %q, want %q", DL3061Code, "hadolint/DL3061")
	}
	if DL3061Message == "" {
		t.Error("DL3061Message should not be empty")
	}
	if DL3061DocURL != "https://github.com/hadolint/hadolint/wiki/DL3061" {
		t.Errorf("DL3061DocURL = %q, want wiki URL", DL3061DocURL)
	}
}
